...
public partial class BillingService : BaseBillingService
{
    ...
    protected override async Task<List<Product>> GetPlatformProductsAsync(List<Product> baseProducts)
    {
        try
        {
            ...
            if (productResult.Result.ResponseCode == BillingResponseCode.Ok)
            {
                ...
                foreach (var baseProduct in baseProducts)
                {
                    ...
                    if (productDetailsDict.TryGetValue(baseProduct.Id, out var productDetails))
                    {
                        ...
                        if (!string.IsNullOrEmpty(formattedPrice)) updatedProduct.Price = formattedPrice;
                        var priceAmount = GetPriceAmount(productDetails);
                        if (priceAmount.HasValue) updatedProduct.PriceAmount = priceAmount.Value;
                        updatedProduct.IsOwned = _ownedProducts.Contains(baseProduct.Id);
                        _logger.LogDebug("Updated product {ProductId}: {Name} - {Price}", updatedProduct.Id, updatedProduct.Name, updatedProduct.Price);
                    }
                    else
                    {
            ...
            {
                _logger.LogError("Failed to query product details: {ResponseCode} {DebugMessage}", productResult.Result.ResponseCode, productResult.Result.DebugMessage);
                foreach (var product in baseProducts) product.IsOwned = _ownedProducts.Contains(product.Id);
                return baseProducts;
            }
        }
        catch (Exception ex)
    ...
    protected override async Task<List<string>> GetPlatformPurchasedProductsAsync()
    {
        try
        {
            ...
            var queryPurchasesParams = QueryPurchasesParams.NewBuilder().SetProductType(BillingClient.ProductType.Inapp).Build();
            var purchaseResponseListener = new PurchasesResponseListener((billingResult, purchases) =>
            {
                if (billingResult.ResponseCode == BillingResponseCode.Ok && purchases != null)
                {
                    ...
                    {
                        if (purchase.PurchaseState == PurchaseState.Purchased) purchasedProducts.AddRange(purchase.Products);
                    }
                }
                tcs.SetResult(purchasedProducts);
            });

            if (_billingClient != null) _billingClient.QueryPurchases(queryPurchasesParams, purchaseResponseListener);
            else
            {
    ...
    protected override async Task<PurchaseResult> PurchasePlatformProductAsync(string productId)
    {
        try
        {
            var productList = QueryProductDetailsParams.Product.NewBuilder().SetProductType("InApp").SetProductId(productId).Build();
            var activity = Platform.CurrentActivity ?? Microsoft.Maui.ApplicationModel.Platform.CurrentActivity;
            if (activity == null)
            {
                return new PurchaseResult { IsSuccess = false, ProductId = productId, ErrorMessage = "No current activity available" };
            }

            var productDetailsParams = QueryProductDetailsParams.NewBuilder().SetProductList(new[] { productList });
            if (_billingClient == null)
            {
                return new PurchaseResult { IsSuccess = false, ProductId = productId, ErrorMessage = "Billing client is not initialized" };
            }
            ...
            var productDetailsParamsList = BillingFlowParams.ProductDetailsParams.NewBuilder().SetProductDetails(skuDetails).Build();
            var billingFlowParams = BillingFlowParams.NewBuilder().SetProductDetailsParamsList(new[] { productDetailsParamsList }).Build();
            var billingResult = _billingClient.LaunchBillingFlow(activity, billingFlowParams);
            if (billingResult.ResponseCode == BillingResponseCode.Ok)
            {
                return new PurchaseResult { IsSuccess = true, ProductId = productId, ErrorMessage = string.Empty };
            }
            return new PurchaseResult { IsSuccess = false, ProductId = productId, ErrorMessage = $"Purchase failed: {billingResult.DebugMessage}" };
        }
        catch (Exception ex)
        {
    ...
    protected override async Task<bool> RestorePlatformPurchasesAsync()
    {
        try
        {
            ...
            foreach (var purchase in restoredPurchases) ProcessPurchase(purchase);
            return restoredPurchases.Count >= 0;
        }
        catch (Exception ex)
        {
    ...
    internal void OnBillingServiceDisconnected() => _logger.LogInformation("Billing service disconnected");
    internal void OnBillingSetupFinished(AndroidBillingResult billingResult) => _logger.LogInformation("Billing setup finished: {ResponseCode} {DebugMessage}", billingResult.ResponseCode, billingResult.DebugMessage);
    internal void OnPurchasesUpdated(AndroidBillingResult billingResult, IList<Purchase>? purchases)
    {
        if (billingResult.ResponseCode == BillingResponseCode.Ok && purchases != null)
        {
            ...
            foreach (var purchase in purchases) ProcessPurchase(purchase);
        }
        ...
        {
            _logger.LogError("Purchase failed: {ResponseCode} {DebugMessage}", billingResult.ResponseCode, billingResult.DebugMessage);
        }
    }
    ...
    private void ProcessPurchase(Purchase purchase)
    {
        try
        {
            ...
            {
                foreach (var productId in purchase.Products) _ownedProducts.Add(productId);
            }
        }
        catch (Exception ex)
        {
    ...
    private decimal? GetPriceAmount(ProductDetails productDetails)
    {
        try
        {
            ...
            if (priceAmountMicros != null) return priceAmountMicros.Value / 1_000_000m;
        }
        catch { }
        return null;
    }
...
internal class BillingClientStateListener : Java.Lang.Object, IBillingClientStateListener
{
    ...
    public BillingClientStateListener(BillingService service) { _service = service; }
    public void OnBillingServiceDisconnected() => _service.OnBillingServiceDisconnected();
    public void OnBillingSetupFinished(AndroidBillingResult billingResult) => _service.OnBillingSetupFinished(billingResult);
}

internal class PurchasesUpdatedListener : Java.Lang.Object, IPurchasesUpdatedListener
{
    private readonly BillingService _service;
    public PurchasesUpdatedListener(BillingService service) { _service = service; }
    public void OnPurchasesUpdated(AndroidBillingResult billingResult, IList<Purchase>? purchases) => _service.OnPurchasesUpdated(billingResult, purchases);
}
...
internal class PurchasesResponseListener : Java.Lang.Object, IPurchasesResponseListener
{
    ...
    public PurchasesResponseListener(Action<AndroidBillingResult, IList<Purchase>> onResponse) { _onResponse = onResponse; }
    public void OnQueryPurchasesResponse(AndroidBillingResult billingResult, IList<Purchase> purchases) => _onResponse(billingResult, purchases);
}