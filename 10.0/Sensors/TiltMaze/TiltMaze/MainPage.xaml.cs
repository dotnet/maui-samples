...
namespace TiltMaze
{
    public partial class MainPage : ContentPage
    {
        ...
        async void TransitionToNewGame()
        {
            ...
            await absoluteLayout.FadeToAsync(0, 1000);
            NewGame((float)absoluteLayout.Width, (float)absoluteLayout.Height);
            await absoluteLayout.FadeToAsync(1, 500);
            isBallInPlay = true;
        }
        ...
        void NewGame(float width, float height)
        {
            ...
            // Get the primary color from resources, fallback to green if not found or not a Color
            Color wallColor = Colors.Green;
            if (Application.Current?.Resources.TryGetValue("Primary", out var primaryObj) == true && primaryObj is Color primaryColor)
                wallColor = primaryColor;
            ...
            BoxView createBoxView() => new BoxView { Color = wallColor };
            for (int x = 0; x < mazeGrid.Width; x++)
                for (int y = 0; y < mazeGrid.Height; y++)
                {
                    MazeCell mazeCell = mazeGrid.Cells[x, y];
                    ...
                    if (mazeCell.HasTop)
                    {
                        Rect rect = new Rect(x * cellWidth - halfWallWidth,
                                             y * cellHeight,
            ...
            absoluteLayout.Children.Add(hole);
            AbsoluteLayout.SetLayoutBounds(hole, new Rect(holePosition.X - HOLE_RADIUS, holePosition.Y - HOLE_RADIUS, 2 * HOLE_RADIUS, 2 * HOLE_RADIUS));
            ...
            AbsoluteLayout.SetLayoutBounds(ball, new Rect(ballPosition.X - BALL_RADIUS, ballPosition.Y - BALL_RADIUS, 2 * BALL_RADIUS, 2 * BALL_RADIUS));
        }
        ...
        bool MoveBall(float deltaSeconds)
        {
            ...
            while (t > 0)
            {
                ...
                if (distanceToCollision < float.MaxValue)
                {
                    ...
                    // Defensive: ensure argument to sqrt is non-negative
                    float sqrtArg = v0.LengthSquared() + 2 * a.Length() * distanceToCollision;
                    if (sqrtArg < 0)
                    {
                        sqrtArg = 0; // fallback to zero
                    }
                    float vMag = (float)Math.Sqrt(sqrtArg);
                    // Defensive: avoid division by zero
                    float aLen = a.Length();
                    float tCollision = 0;
                    if (aLen == 0)
                    {
                        tCollision = 0.001f; // fallback to small step
                    }
                    else
                    {
                        tCollision = (vMag - v0.Length()) / aLen;
                    }
                    // Defensive: avoid NaN in Vector2.Normalize
                    Vector2 vNorm = v0;
                    if (v0.LengthSquared() == 0)
                    {
                        vNorm = Vector2.UnitX; // fallback
                    }
                    else
                    {
                        vNorm = Vector2.Normalize(v0);
                    }
                    v = vMag * vNorm;
            ...
            // Prevent NaN propagation: if ballPosition is NaN, reset to center
            if (float.IsNaN(ballPosition.X) || float.IsNaN(ballPosition.Y))
            {
                ballPosition = new Vector2((float)absoluteLayout.Width / 2, (float)absoluteLayout.Height / 2);
                ballVelocity = Vector2.Zero;
                Debug.WriteLine("[WARN] ballPosition was NaN. Resetting to center.");
            }

            // Clamp ball position to stay within the maze bounds
            ballPosition.X = Math.Clamp(ballPosition.X, BALL_RADIUS, (float)absoluteLayout.Width - BALL_RADIUS);
            ballPosition.Y = Math.Clamp(ballPosition.Y, BALL_RADIUS, (float)absoluteLayout.Height - BALL_RADIUS);

            // Position the ball at ballPosition
            Rect ballRect = new Rect(ballPosition.X - BALL_RADIUS,
