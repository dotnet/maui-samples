...
<#
.SYNOPSIS
    Configures (and optionally hosts a dev tunnel for) the .NET MAUI Passkeys sample,
    which is served by the src/Passkeys.Server relying-party web app.
...
.DESCRIPTION
    ...
    Into the git-ignored src/Passkeys.Client/Passkeys.Local.props (and, for Apple,
    src/Passkeys.Client/Platforms/iOS/
...
.PARAMETER TunnelId
    The dev tunnel id/name to create or reuse. Defaults to 'maui-samples-passkeys'. Keep it constant to
...
[CmdletBinding()]
param(
    [string]$TunnelId = 'maui-samples-passkeys',
    [int]$Port = 5177,
...
$project = Join-Path $here 'src' 'Passkeys.Server' 'Passkeys.Server.csproj'
...
function Assert-NativeSuccess($operation) {
...
function Get-AppleTeamId {
    ...
    foreach ($f in Get-AppleProvisioningProfileFiles) {
...
function Get-AppleProvisioningProfileFiles {
    # Xcode 16+ writes profiles under UserData. Keep the legacy MobileDevice location for profiles
    # installed by older Xcode versions or manually. New Xcode profiles take precedence.
    $directories = @(
        (Join-Path $HOME 'Library' 'Developer' 'Xcode' 'UserData' 'Provisioning Profiles'),
        (Join-Path $HOME 'Library' 'MobileDevice' 'Provisioning Profiles')
    )
    foreach ($dir in $directories | Select-Object -Unique) {
        if (Test-Path $dir) {
            Get-ChildItem -Path (Join-Path $dir '*') -Include '*.provisionprofile', '*.mobileprovision' -File -ErrorAction SilentlyContinue
        }
    }
}

# Scans installed macOS provisioning profiles for a non-expired profile whose application-identifier
# equals <appId> (explicit) and that carries the associated-domains entitlement. Returns its Name.
function Find-AppleProvisioningProfile($appId) {
    ...
    foreach ($f in Get-AppleProvisioningProfileFiles) {
        ...
        $platform = [regex]::Match($xml, '<key>Platform</key>\s*<array>(.*?)</array>', [System.Text.RegularExpressions.RegexOptions]::Singleline)
        if (-not $platform.Success -or $platform.Groups[1].Value -notmatch '<string>(OSX|macOS)</string>') { continue }
        $expiration = [regex]::Match($xml, '<key>ExpirationDate</key>\s*<date>([^<]+)</date>')
        if ($expiration.Success -and [DateTime]::Parse($expiration.Groups[1].Value).ToUniversalTime() -le [DateTime]::UtcNow) { continue }
...
# Writes the git-ignored src/Passkeys.Client/Passkeys.Local.props by loading the committed
# Passkeys.Local.in.props
...
function Write-PasskeysLocalProps($appDir, $serverUrl, $applicationId, $iosEntitlementsRel, $macEntitlementsRel, $identity, $profileName) {
    ...
    foreach ($n in @($project.GetElementsByTagName('PasskeysApplicationId'))) { $n.InnerText = $applicationId }

    foreach ($pg in @($project.GetElementsByTagName('PropertyGroup'))) {
        $entitlements = $pg.GetElementsByTagName('CodesignEntitlements')
        if ($entitlements.Count -gt 0) {
            $isMacCatalyst = $pg.GetAttribute('Condition') -match 'maccatalyst'
            $entitlementsRel = if ($isMacCatalyst) { $macEntitlementsRel } else { $iosEntitlementsRel }
            if ($entitlementsRel) {
                foreach ($n in @($entitlements)) { $n.InnerText = $entitlementsRel }
            }
            else {
    ...
    if ($identity -and $profileName) {
        ...
        # No Apple signing: keep simulator entitlements, but drop the device/Mac Catalyst signing group.
        $appleProps = @('CodesignKey', 'CodesignProvision', 'MtouchLink')
        foreach ($pg in @($project.GetElementsByTagName('PropertyGroup'))) {
            $isApple = $false
...
if ($loggedInUser) {
    ...
    Assert-NativeSuccess 'Dev tunnel login'
}
...
if (-not $tunnelJson.tunnel) {
    ...
    Assert-NativeSuccess "Creating dev tunnel '$TunnelId'"
    $tunnelJson = devtunnel show $TunnelId --json 2>$null | ConvertFrom-Json
}
else {
    # Apple and Android fetch association documents without a tunnel login. Existing tunnels might
    # predate this sample or have private defaults, so verify their access instead of assuming it.
    $accessJson = (devtunnel access list $TunnelId --json 2>$null) -join "`n"
    if ($accessJson -notmatch '(?i)anonymous|"\*"') {
        Write-Host "    Enabling anonymous client access for platform association checks…" -ForegroundColor DarkGray
        devtunnel access create $TunnelId --anonymous | Out-Host
        Assert-NativeSuccess "Enabling anonymous access on '$TunnelId'"
    }
}
if (-not ($tunnelJson.tunnel.ports | Where-Object { $_.portNumber -eq $Port })) {
    ...
    Assert-NativeSuccess "Creating port $Port on '$TunnelId'"
}
...
# Reset every script-owned collection entry first. This makes -NoAndroid/-NoApple deterministic and
# prevents values from an earlier run from remaining trusted or published by the server.
$managedSecretKeys = @(
    'Passkeys:AllowedOrigins:0',
    'Passkeys:AllowedOrigins:1',
    'Passkeys:Android:PackageName',
    'Passkeys:Android:Sha256CertFingerprints:0',
    'Passkeys:Apple:AppIds:0'
)
foreach ($key in $managedSecretKeys) {
    dotnet user-secrets --project $project remove $key 2>$null | Out-Null
    Assert-NativeSuccess "Removing stale user-secret '$key'"
}
dotnet user-secrets --project $project set 'Passkeys:ServerDomain' $domain | Out-Null
Assert-NativeSuccess 'Writing Passkeys:ServerDomain'
dotnet user-secrets --project $project set 'Passkeys:AllowedOrigins:0' $uri | Out-Null
Assert-NativeSuccess 'Writing the web allowed origin'
...
elseif (-not $AppleTeamId) {
    ...
    Assert-NativeSuccess 'Writing the Apple app ID'
    ...
    $iosBaseEnt = Join-Path $appDir 'Platforms' 'iOS' 'Entitlements.plist'
    $iosLocalEnt = Join-Path $appDir 'Platforms' 'iOS' 'Entitlements.Local.plist'
    if (New-LocalEntitlements $iosBaseEnt $iosLocalEnt $domain) {
        $iosEntitlementsRel = 'Platforms/iOS/Entitlements.Local.plist'
    }
    $macBaseEnt = Join-Path $appDir 'Platforms' 'MacCatalyst' 'Entitlements.plist'
    $macLocalEnt = Join-Path $appDir 'Platforms' 'MacCatalyst' 'Entitlements.Local.plist'
    if (New-LocalEntitlements $macBaseEnt $macLocalEnt $domain) {
        $macEntitlementsRel = 'Platforms/MacCatalyst/Entitlements.Local.plist'
    }
    Write-Host "      entitlement: webcredentials:$domain  (platform-local entitlements — git-ignored)" -ForegroundColor DarkGray
...
$propsPath = Write-PasskeysLocalProps $appDir $uri $ApplicationId $iosEntitlementsRel $macEntitlementsRel $resolvedIdentity $resolvedProfile
